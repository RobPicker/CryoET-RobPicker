...
def score(
    ...
    if cfg is not None:
        particle_radius = dict(cfg.particle_radi)
        weights = dict(getattr(cfg, "metric_weights", {k: 1 for k in particle_radius}))
    else:
        particle_radius = {
            'apo-ferritin': 60,
            'beta-amylase': 65,
    ...
    if solution.duplicated(subset=['experiment', 'x', 'y', 'z']).sum() != 0:
        # print("Warning: Duplicates found in solution data.")
        # print(solution[solution.duplicated(subset=['experiment', 'x', 'y', 'z'], keep=False)])
        # Remove duplicates in solution
        solution = solution.drop_duplicates(subset=['experiment', 'x', 'y', 'z']).copy()
    ...
    for p in particles:
        ...
        for c in ths:
            scores += [score(
                        ...
                        beta=getattr(cfg, "metric_beta", 4),
                        weighted=False,
                        cfg=cfg)[0]]
